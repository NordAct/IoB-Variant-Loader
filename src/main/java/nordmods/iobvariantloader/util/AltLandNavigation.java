...
public class AltLandNavigation<T extends ADragonBase> extends FlyingPathNavigation {
    ...
    public AltLandNavigation(T mobEntity, Level world) {
    ...
    @Override
    public void tick() {
        if (entity.canBeControlledByRider() || entity.isPassenger() || entity.isOrderedToSit()) return;

        boolean isFullBlock = entity.getFeetBlockState().isCollisionShapeFullBlock(entity.level, entity.getOnPos());
        if (GoalUtils.isSolid(entity, entity.blockPosition()) && isFullBlock) entity.getJumpControl().jump();
        entity.setPathfindingMalus(BlockPathTypes.WATER, !hasTargetInWater() ? 8 : 0);
        entity.setPathfindingMalus(BlockPathTypes.WATER_BORDER, !hasTargetInWater() ? 8 : 0);
    ...
    protected void followThePath() {
        ...
        boolean bl = xDiff < (double)maxDistanceToWaypoint && zDiff < (double)maxDistanceToWaypoint &&  yDiff <= entity.getStepHeight() && yDiff > -entity.getMaxFallDistance();
    ...
    protected boolean shouldTargetNextNodeInDirection(Vec3 currentPos) {
        ...
        if (!nodeChecked) {
            ...
            for (BlockPos pos : toCheck) {
                if (entity.getPathfindingMalus(nodeEvaluator.getBlockPathType(level, pos.getX(), pos.getY(), pos.getZ())) == 0) continue;
    ...
    protected boolean hasTargetInWater() {
        return (getTargetPos() != null && level.getBlockState(getTargetPos()).getMaterial().isLiquid()
                || entity.getTarget() != null && entity.getTarget().isUnderWater());
    }

    public boolean canCutCorner(final BlockPathTypes pathType) {
        return pathType != BlockPathTypes.DANGER_CACTUS && pathType != BlockPathTypes.DAMAGE_OTHER && pathType != BlockPathTypes.WALKABLE_DOOR;
    }
}
